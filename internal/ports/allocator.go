...
func (a *ClusterAllocator) Allocate(ctx context.Context, gs *v1alpha1.GameServer) ([]v1alpha1.PortSpec, error) {
	...
	// Count is modeled as the number of additional ports after StartPort, so the
	// reserved range size always includes the first port itself. Example:
	// start=30005,count=5 reserves 30005-30010.
